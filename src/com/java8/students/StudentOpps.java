...
public class StudentOpps {
    public static void main(String[] args) {
        ...
        List<Student> filteredList = studentList.stream().filter(s -> s.getRank() >= 50 && s.getRank() <= 100).collect(Collectors.toList());
        //System.out.println("List of students whose rank is in between 50 and 100: " + filteredList);

        //2. Find the Students who stays in Karnataka and sort them by their names
        studentList.stream().filter(s -> s.getCity().equals("Karnataka")).sorted((s1, s2) -> s1.getFirstName().compareTo(s2.getFirstName())).forEach(System.out::println);
        ...
        studentList.stream()
                ...
                .forEach((dept, student) -> System.out.println("Highest rank in " + dept + " is " + student.get().getRank()));
